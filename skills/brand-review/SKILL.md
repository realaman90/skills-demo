...
3. If `get_skill_file` or `search_docs` are unavailable in the session, use `run_tool` fallback:
   - `run_tool` with `tool_name="get_skill_file"` and `args={"skill_id":"brand-review","path":"references/INDEX.md"}`
   - `run_tool` with `tool_name="search_docs"` and `args={"skill_id":"brand-review","query":"<keywords>","top_k":5}`
...
VERIFICATION STRING: VERIFIED_SKILL:brand-review:v1
Final answer must start with: VERIFIED_SKILL:brand-review:v1
